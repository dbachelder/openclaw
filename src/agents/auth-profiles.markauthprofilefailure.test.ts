...
describe("markAuthProfileFailure", () => {
  ...
  it("resets error count when previous cooldown has expired to prevent escalation", async () => {
    ...
    // previous cooldown expired. Cooldown should be ~30 min.
    expect(stats?.errorCount).toBe(1);
    expect(stats?.failureCounts?.rate_limit).toBe(1);
    const cooldownMs = (stats?.cooldownUntil ?? 0) - now;
    // calculateAuthProfileCooldownMs(1) = 30 minutes
    // Roomie fork: stepped backoff tuned to ~30 min (see usage.ts calculateAuthProfileCooldownMs).
    // Upper bound has a small margin because the cooldown is anchored to Date.now() inside
    // markAuthProfileFailure, which runs a few ms after the `now` captured above.
    expect(cooldownMs).toBeLessThanOrEqual(30 * 60_000 + 5_000);
    expect(cooldownMs).toBeGreaterThan(29 * 60_000);
  });
...
describe("calculateAuthProfileCooldownMs", () => {
  it("uses a flat 30-minute cooldown", () => {
    expect(calculateAuthProfileCooldownMs(1)).toBe(30 * 60_000); // 30 minutes
    expect(calculateAuthProfileCooldownMs(2)).toBe(30 * 60_000); // 30 minutes
    expect(calculateAuthProfileCooldownMs(3)).toBe(30 * 60_000); // 30 minutes
    expect(calculateAuthProfileCooldownMs(4)).toBe(30 * 60_000); // 30 minutes
    expect(calculateAuthProfileCooldownMs(5)).toBe(30 * 60_000); // 30 minutes
  });
});
